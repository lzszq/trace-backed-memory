...
| Governed Artifact retention v1 | `artifact_retention_event_v1.py`, `managed_index_v3.py`, generated retention event Schema/example | protected redaction manifest, hold-epoch authorization, exact external KMS receipt verification, immutable index-successor CAS, replay-partial sidecar, crypto-erasure/tombstone ordering and crash recovery tests, bilingual protocol docs; coordinator only, not a new authority or default adapter |
| FailureCase events and structured evidence v1 | `failure_case_event_v1.py`, generated FailureCase event Schema/example | exact TraceEvent/proposal linkage, candidate-only extractor, independent review, FixEvidence/StructuredRegressionEvidence transitions, legacy-unstructured downgrade, deterministic projection tests, bilingual protocol docs; opt-in input for later MemoryCatalog reducer |
