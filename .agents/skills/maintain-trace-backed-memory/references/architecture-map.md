...
| Completion outbox v3 | `completion_outbox_v3.py`, `sqlite_completion_outbox_v3.py`, `postgres_completion_outbox_v3.py`, `schemas/sqlite-v3-completion-outbox.sql`, `schemas/postgres-v3-completion-outbox*.sql` | atomic completion/event insert, append-only delivery revisions, lease/retry/dead-letter transitions, exact replay, schema/catalog-drift/rollback/savepoint/concurrency tests, protocol docs |
