...
- Postgres schema parity checks for model defaults, an append-only shared runtime memory ID namespace, non-empty required text, composite case/trace commit provenance, failure-case/lesson lifecycle cascades and guards, JSONB object/array and element-type checks, required usage-decision audit evidence, and context example parsing.
