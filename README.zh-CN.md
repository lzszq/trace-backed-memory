...
当前白名单包含 81 项资源。`PackagedResource` 描述包含资源种类、媒体类型、字节数和 SHA-256。`load_failure_taxonomy()` 默认加载包内规范分类体系；传入路径时仍会加载调用方拥有的文件。
...
发布权限。详见[证据契约](docs/protocols/evidence-v3.zh-CN.md)。
...
详见[revision 契约](docs/protocols/memory-revision-v3.zh-CN.md)。
...
- 严格 JSON 快照、简单 active lesson YAML、81 项 zip-safe 包资源和原子文件发布。
