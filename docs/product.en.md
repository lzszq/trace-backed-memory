...
| Immutable revision publication preparation | Content-derived MemoryRevision proposals bind stable identity, exact parent, artifact, scope, evidence, and proposer context; isolated SQLite/PostgreSQL ledgers persist verified proposal bundles, while separate approval/activation events re-verify exact authorization, actors, bytes, evidence, and predecessor linkage; durable current-head locking, publication persistence, and active-v2 projection remain separate service work |
...
| Distribution resources | 119 byte-identical packaged Schemas, SQL and migration files, taxonomy files, and examples with discovery, exact-byte reads, metadata, and export |
