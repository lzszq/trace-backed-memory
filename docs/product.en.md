...
| Replayable retrieval preparation | Content-addressed policy and optional authenticated preparation kernel authorize first, load verified activated revisions, filter classification/applicability/eval leakage/Git ancestry, deterministically fuse versioned adapter scores, and emit paired RetrievalSnapshot/System Gate evidence with final head/policy rechecks; managed indexes, Semantic Gate, GateSession persistence, and active adapter wiring remain outstanding |
...
| Distribution resources | 127 byte-identical packaged Schemas, SQL and migration files, taxonomy files, and examples with discovery, exact-byte reads, metadata, and export |
