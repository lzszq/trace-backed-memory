...
| Replayable gate preparation | Content-derived System Gate evaluations and Semantic Gate attempts bind deterministic rule outcomes and provider/model/prompt/response provenance while enforcing that a model can only narrow; opt-in SQLite/PostgreSQL ledgers persist one immutable linear attempt chain, while active policy does not emit it |
| Distribution resources | 97 byte-identical packaged Schemas, SQL and migration files, taxonomy files, and examples with discovery, exact-byte reads, metadata, and export |
