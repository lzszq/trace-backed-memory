...
| Replayable retrieval preparation | Content-addressed policy and optional authenticated preparation kernel authorize first, load verified activated revisions, filter classification/applicability/eval leakage/Git ancestry, deterministically fuse scores, and emit paired RetrievalSnapshot/System Gate evidence with final head/policy rechecks; an opt-in immutable five-view managed index bundle supplies content-addressed metadata/lexical/semantic/evidence/Git discovery through exact SQLite/PostgreSQL scope-head CAS; a durable composition service now creates the authorized GateSession, stores and verifies the exact evidence pair, and CAS-publishes `PREPARED`, while Semantic Gate, production sharding/workers, and active adapter wiring remain outstanding |
...
| Distribution resources | 132 byte-identical packaged Schemas, SQL and migration files, taxonomy files, and examples with discovery, exact-byte reads, metadata, and export |
...
The current release implements roadmap Phases 0 through 73, the local agent/MCP integration increment, and the delivered contract/isolated-authority portions of Phase 74, including opt-in SQLite and isolated PostgreSQL completion outboxes plus their bounded at-least-once delivery worker, the bounded five-view managed-index bundle with exact SQLite/PostgreSQL publication authorities, and the opt-in durable retrieval-preparation bridge through Gate evidence verification and GateSession `PREPARED`. The main product path has executable README examples, JSON Schemas, SQL invariants, and pytest coverage across Python 3.11, 3.12, 3.13, Windows, SQLite, and required PostgreSQL CI.
...
- Gate requests and finalized tombstones remain process-local. Pending requests
  ...
  `PREPARED`; transport authentication, Semantic Gate and later transitions,
  shared-service workers, and active adapter integration remain out of scope.
