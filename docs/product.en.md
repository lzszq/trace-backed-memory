...
| Agent application boundary | `LocalAgentMemory`, Git-backed Trace capture, stable errors, `tbm capabilities`, versioned `tbm.agent.v1` schemas/examples, one strict MCP/HTTP dispatcher, optional long-running local STDIO MCP and loopback HTTP, and a dependency-free typed Python HTTP client |
...
| Distribution resources | 136 byte-identical packaged Schemas, SQL and migration files, taxonomy files, and examples with discovery, exact-byte reads, metadata, and export |
...
The local loopback HTTP adapter and typed Python SDK expose the active
version-2 lifecycle through the same strict dispatcher as MCP. Their bearer
secret protects a local process boundary; it is not the service-identity
transport authentication required by the durable v3 composition. Remote or
shared deployment, TypeScript SDK support, and durable-facade adapter wiring
remain outstanding.
