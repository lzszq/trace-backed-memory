...
| Durable completion preparation | A storage-neutral completion service and opt-in SQLite authority atomically bind one content-addressed RunOutcome to the `EXECUTING` → `COMPLETED` GateSession revision with trusted time, CAS, exact replay, and read-back; PostgreSQL parity and active runtime integration remain outstanding |
| Distribution resources | 103 byte-identical packaged Schemas, SQL and migration files, taxonomy files, and examples with discovery, exact-byte reads, metadata, and export |
