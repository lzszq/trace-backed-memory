...
| Operations CLI | Dependency-free `tbm` and module entry point for snapshots, v3 migration preflight/bundle/apply/verify/rollback, lessons, obsolescence, audits, metrics, PR reports, completion, and recovery |
...
| Local SQLite v3 migration | Explicit snapshot-v2/SQLite-v1 apply, full verification, source backup, immutable per-record legacy-evidence disposition, and compat-v2 rollback; old active status is never promoted into v3 publication |
| Migration preparation | Content-addressed v2-to-v3 bundles, exact plan replay, immutable SQLite migration ledger, and version-gated PostgreSQL staging/rollback |
| Unified SQLite v3 persistence | Opt-in generated 16-component bundle installs the complete durable authority and migration catalog in one transaction and verifies exact component versions plus all controlled main/temp tables, indexes, automatic indexes, and triggers; compatibility SQLite remains schema version 1 |
