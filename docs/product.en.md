...
| Durable completion preparation | A storage-neutral completion service plus opt-in SQLite and isolated PostgreSQL authorities atomically bind one content-addressed RunOutcome to the `EXECUTING` → `COMPLETED` GateSession revision; companion immutable OutcomeAttribution ledgers retain multiple verified association/causal claims with exact linkage, replay, and read-back; active runtime integration remains outstanding |
| Distribution resources | 108 byte-identical packaged Schemas, SQL and migration files, taxonomy files, and examples with discovery, exact-byte reads, metadata, and export |
