...
| Replayable gate preparation | Content-derived System Gate evaluations and Semantic Gate attempts bind deterministic rule outcomes and provider/model provenance while enforcing that a model can only narrow; exact prompt/response byte bindings verify role digests, and opt-in SQLite/PostgreSQL ledgers persist one immutable linear attempt chain, while active policy does not emit it |
| Distribution resources | 99 byte-identical packaged Schemas, SQL and migration files, taxonomy files, and examples with discovery, exact-byte reads, metadata, and export |
