...
| 持久化完成准备 | storage-neutral completion service、opt-in SQLite authority 与隔离 PostgreSQL authority 把一份 content-addressed RunOutcome 与 `EXECUTING` → `COMPLETED` GateSession revision 原子绑定；配套 immutable OutcomeAttribution ledger 保留多条已核验 association/causal claim；opt-in SQLite 与隔离 PostgreSQL completion outbox 原子增加一条 immutable event 与 append-only leased retry/dead-letter delivery chain；active runtime integration 仍待完成 |
