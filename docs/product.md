...
| 不可变 revision publication 准备 | 内容派生 MemoryRevision proposal 绑定 stable identity、精确 parent、artifact、scope、evidence 与 proposer context；隔离 SQLite/PostgreSQL ledger 持久化已验证 proposal bundle，独立 approval/activation event 则重新验证精确 authorization、actor、字节、evidence 与 predecessor linkage；durable current-head 锁、publication 持久化和 active-v2 projection 仍是独立 service 工作 |
| 可回放检索准备 | 内容派生 RetrievalSnapshot 记录已授权 context/query 摘要、retriever/index 版本、有序不可变 revision 命中、候选哈希、有限 stage/fusion 分数、边界与截断原因；active retrieval 尚不产生它 |
| 可回放门禁准备 | 内容派生 System Gate evaluation 与 Semantic Gate attempt 绑定确定性 rule 结果及 provider/model provenance，并强制模型只能缩小；精确 prompt/response binding 核验角色 digest，opt-in SQLite repository 原子保存 public/internal 字节，SQLite/PostgreSQL ledger 持久化一条不可变线性 attempt chain，active policy 尚不产生它 |
...
| 分发资源 | wheel/sdist/editable 内置 119 份 byte-identical Schema、SQL/迁移、taxonomy 与示例；支持发现、读取、校验元数据和原子导出 |
