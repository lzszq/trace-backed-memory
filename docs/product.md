...
显式 durable runtime 还可以选择 server-owned Semantic provider effect invoker。可信
reconciliation、owner-fence verification 与有界 retry/dead-letter 只有在配置对应依赖时才激活。
...
显式 durable runtime 已选择 server-owned Semantic invocation；配置后，可信 reconciliation
