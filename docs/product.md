...
| Agent 应用边界 | `LocalAgentMemory`、Git-backed Trace capture、稳定错误、`tbm capabilities`、带版本的 `tbm.agent.v1` Schema/示例、MCP/HTTP 共用的严格 dispatcher、可选长驻本地 STDIO MCP 与 loopback HTTP，以及无依赖类型化 Python HTTP client |
...
| 重放持久化准备 | 与存储实现无关的内容寻址 artifact、精确 injection 与固定 component decision-manifest v3 契约，以及 opt-in 隔离 SQLite/PostgreSQL immutable 字节/descriptor 账本；active adapter 尚不使用它们 |
...
| 分发资源 | wheel/sdist/editable 内置 136 份 byte-identical Schema、SQL/迁移、taxonomy 与示例；支持发现、读取、校验元数据和原子导出 |
