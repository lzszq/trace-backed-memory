...
| 重放持久化准备 | 与存储实现无关的内容寻址 artifact、精确 injection、固定 component decision-manifest 与可移植 `tbm.replay-export.v3` 契约，以及 opt-in 隔离 SQLite/PostgreSQL immutable 字节/descriptor 账本；durable facade 现在会用新的 `artifact:read` decision 授权精确版本、session-bound 的直接 Python export，active transport 尚不暴露它 |
...
| 持久化执行与完成准备 | opt-in authenticated execution 组合会在 `FINALIZED` → `EXECUTING` 前核验已保留 injection，支持精确 revision 的 lease resume 与 abandonment，认证已注册 outcome evaluator，并组合 opt-in SQLite 或隔离 PostgreSQL authority，原子绑定一份 content-addressed RunOutcome、`COMPLETED`、一条 immutable completion event 与 append-only leased retry/dead-letter delivery chain；配套 OutcomeAttribution ledger 与有界 outbox worker 保留已核验 association/causal claim 和 at-least-once delivery；active adapter integration 仍待完成 |
| 分发资源 | wheel/sdist/editable 内置 149 份 byte-identical Schema、OpenAPI、SQL/迁移、taxonomy 与示例；支持发现、读取、校验元数据和原子导出 |
...
以下 Phase 49–70 段落保留各功能落地时的版本与资源基线，属于历史兼容记录，其中出现的资源数量仍按各阶段当时的值解释，不随当前发布回写。当前发布契约以 PostgreSQL schema version 2 和 149 项资源说明为准。历史上的第 21 项资源是面向既有 version-2 数据库、可重复运行且带版本门禁的 `schemas/postgres-v2-lock-order-hotfix.sql`；当前 `tbm.agent.v1` 资源包含十一份 JSON Schema、一份 OpenAPI、十一份 JSON 示例与一份可执行 quickstart；v3 资源包含 migration mapping/plan preflight、不可激活 bundle、隔离的 SQLite/PostgreSQL staging DDL、显式 PostgreSQL staging rollback、GateSession/内容寻址 injection/replay manifest/replay export 契约 Schema/示例、MemoryRevision approval/activation Schema/示例、隔离 SQLite MemoryRevision publication、GateSession/replay/audit/MemoryRevision/Artifact Authority/Semantic Gate/RunOutcome ledger DDL、隔离 PostgreSQL GateSession、MemoryRevision publication、replay、audit、authorization、Artifact Authority、Semantic Gate 与 RunOutcome ledger install/fail-closed rollback，以及授权 policy/decision Schema/示例。全新安装与当前 v1→v2 迁移已包含同一数据库修复。
