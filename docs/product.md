...
| 运维 CLI | dependency-free `tbm` / `python -m trace_backed_memory`；snapshot validate/stats、v3 migration preflight/bundle/apply/verify/rollback、active lessons 原子导出与 dry-run 导入、failure case/lesson/project policy forward-only 淘汰预览与显式写入、audit/metrics/remediation、只读 PR report、单项与清单式批量 measured completion、dry-run 恢复与显式 `--write` 原子替换 |
...
| 本地 SQLite v3 迁移 | 显式 snapshot-v2/SQLite-v1 apply、完整 verify、源备份、immutable 逐记录 legacy-evidence disposition 与 compat-v2 rollback；旧 active status 不会被提升为 v3 publication |
| 迁移准备 | content-addressed v2→v3 bundle、精确 plan replay、immutable SQLite migration ledger，以及 PostgreSQL version-gated staging/rollback |
| 统一 SQLite v3 持久化 | opt-in 生成式 16-component bundle 在一个事务中安装完整 durable authority 与 migration catalog，并校验精确 component version 以及全部受控 main/temp table、index、automatic index 与 trigger；兼容 SQLite 保持 schema version 1 |
...
| 重放持久化准备 | 与存储实现无关的内容寻址 artifact、精确 injection、固定 component decision-manifest 与可移植 `tbm.replay-export.v3` 契约，以及 opt-in 隔离 SQLite/PostgreSQL immutable 字节/descriptor 账本；durable facade 与显式 durable HTTP profile 会用新的 `artifact:read` decision 授权精确版本、session-bound export，response bytes 仍默认关闭 |
