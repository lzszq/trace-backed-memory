...
| 重放持久化准备 | 与存储实现无关的内容寻址 artifact、精确 injection、固定 component decision-manifest 与可移植 `tbm.replay-export.v3` 契约，以及 opt-in 隔离 SQLite/PostgreSQL immutable 字节/descriptor 账本；有界导出要求显式 classification allowlist，active adapter 尚不使用它 |
