...
- 当前版本：`0.1.0`（Alpha）
- 交付形态：Python 库 + `tbm` CLI + 可选本地 MCP/HTTP adapter + JSON/YAML/JSON Schema + SQLite + 可选 PostgreSQL Repository
- 运行要求：Python 3.11+（标准库 SQLite）；PostgreSQL 能力要求 PostgreSQL 12+
- 开源协议：MIT

能力的权威 `active` / `opt-in` / `contract-only` / `planned` 分类见
[当前能力状态账本](status/current-capability-matrix.zh-CN.md)。历史 phase 中的
“已实现”不代表默认 transport 已经选择该能力。
...
| Durable Agent adapter 边界 | `tbm.durable-agent-wire.v1` 严格 request model 与 dispatcher 会映射完整 authenticated durable facade，不接受调用方 identity 字段或进程内 handle；显式 `tbm-http --profile durable-v3`、可信本地 `tbm-mcp --profile durable-v3` 与 `tbmd local` 会通过 operator 持有的 application factory 与统一 runtime factory 选择它；同步/异步 Python 与 Node.js TypeScript client 会选择 HTTP profile，兼容 CLI 不会选择 |
...
| 本地 durable service | `tbmd init/local/doctor/health` 持有一个带锁、由 owner 控制的 SQLite v3 graph，让有界 STDIO MCP 与 loopback HTTP 共用一套 dispatcher，运行有界 GateSession recovery 与 completion-outbox delivery，并按顺序关闭 worker、transport 与 runtime |
...
| 统一 SQLite v3 持久化 | opt-in 生成式 15-component bundle 在一个事务中安装完整非迁移 authority catalog，并校验精确 component version 以及全部受控 main/temp table、index、automatic index 与 trigger；active SQLite v1 transport 保持不变 |
...
| 重放持久化准备 | 与存储实现无关的内容寻址 artifact、精确 injection、固定 component decision-manifest 与可移植 `tbm.replay-export.v3` 契约，以及 opt-in 隔离 SQLite/PostgreSQL immutable 字节/descriptor 账本；显式 durable finalization 会 append canonical final-decision/injection event，durable facade 会从 ledger 重建 session-bound export metadata，再在新的 `artifact:read` decision 下从 authenticated replay authority 读取精确字节；response bytes 仍默认关闭 |
...
显式 durable SQLite/PostgreSQL runtime 会在 finalization 时依次 append
`tbm.usage_decision.finalized` 与 `tbm.injection.rendered`，并在同一个 transaction 中写
`FINALIZED` 与 replay projection。session replay 会从 ledger 重建 canonical metadata，
再只从 authenticated replay authority 读取精确字节；默认 compatibility 行为不变。
...
transport authentication。显式 durable HTTP 与可信本地 MCP 会选择 durable facade；
同步/异步 Python client 与 Node.js `DurableAgentHTTPClient` 会选择显式 HTTP
profile；远程/共享部署、CLI durable 选择与默认 profile cutover 仍待完成。
...
发布。transport authentication、受保护内容加密、其余 lifecycle transition-event linkage、
...
它不是默认 Agent/MCP path，也不自行提供 transport authentication。显式 durable HTTP
与可信本地 MCP adapter 会在各自 transport 边界下选择它。
...
migration 或 bundle coordinator，并拒绝未登记 authority。F1 增加 opt-in SQLite
与隔离 PostgreSQL event-ledger 后端，覆盖精确 Artifact descriptor、原子 append/replay、
integrity/catalog 校验、backup/rollback 和跨后端一致性。F1 基础本身与默认
compatibility path 不会选择它们；显式 durable F2 lifecycle slice 现已选择。F1 还交付
opt-in `tbm.reducer.v1` runtime：versioned descriptor 与 sealed registry、
...
F2 现已进入实施阶段。显式 durable SQLite/PostgreSQL runtime 会启用 event-first
GateSession revision：每条 canonical event 与同步 revision-row projection 共享同一
transaction，typed reducer 可重建精确 current session。SQLite 已覆盖持久 checkpoint
resume、shadow comparison、activation、rollback 与逐字段 parity。durable retrieval
preparation 还会先 append 紧凑、绑定 exact digest/Artifact ref 的 RetrievalSnapshot 与
SystemGateEvaluation events，再写现有 evidence rows；pure current-linkage reducer 与
原子 rollback 测试已经覆盖该路径。详见
[Gate Evidence Event v1](protocols/gate-evidence-event-v1.zh-CN.md)。Semantic Gate attempt
现会先 append 紧凑 failed/succeeded event，再写 attempt/exact-byte projection；它要求已
保留的 System Gate parent，并以精确 event/authority parity 重建。详见
[Semantic Gate Attempt Event v1](protocols/semantic-gate-attempt-event-v1.zh-CN.md)。
finalization 现会在同一 transaction 内 append `UsageDecisionFinalized` 与
`InjectionRendered`，并同步发布 `FINALIZED` 和 replay projection；
`final-decision-injection` reducer 会重建精确 decision、injection、manifest、Artifact
role 与 event-head linkage。显式 durable replay export 现从 ledger 派生 metadata，只从
authenticated replay authority 加载精确字节，并核验 projection parity。详见
[Finalization Event v1](protocols/finalization-event-v1.zh-CN.md) 与
[Ledger Replay Export v1](protocols/ledger-replay-export-v1.zh-CN.md)。typed outcome/
attribution 与本地 completion-effect event/reducer 已覆盖精确 RunOutcome、
OutcomeAttribution、delivery-history 与 dead-letter parity。provider receipt、
unknown-result reconciliation、durable compensation、完整 transport conformance 与 F2
kill matrix 仍未完成。因此 ledger 尚未成为
...
当前仍有明确的生产边界：snapshot version 2 没有 canonical `repository_id` 或显式 global/repository/tenant scope kind；独立的授权 v3 准备契约不会把现行 scope matching 变成多租户授权；`regression_passed` 仍不是结构化 run/evaluator 证据；默认兼容 profile 的 Gate request 与 finalized tombstone 仍只存在于进程内，但 pending request 已有容量限制和显式取消，finalized tombstone 已有容量限制，高层请求会绑定 Trace/run，最终 usage log 会持久化 `request_id`；与存储实现无关的 replay descriptor 已定义 retriever/index、Gate prompt/response、ancestry、policy、renderer 与精确 snippet hash，opt-in SQLite replay ledger 与隔离 PostgreSQL repository 均可保存并复验精确字节/descriptor，PostgreSQL 同时提供 idempotency、savepoint、drift 与 concurrency 对等，opt-in finalization 组合会授权并复查一个精确 scope、原子保留完整 component bundle，并把它关联到 `FINALIZED`；durable facade 已能授权 session-bound replay read，显式 durable HTTP/MCP 与 Python/TypeScript client 只会在启动 content policy 允许时暴露；active-v2 usage-log projection、受保护内容 retention/encryption 与 shared-service transport 仍待完成；Git ancestry 仍是 opt-in。version-3 GateSession 已有 opt-in SQLite 与隔离 PostgreSQL revision repository，但不是默认 Agent/MCP state；默认 Agent/MCP profile 不调用认证 retrieval kernel，可选本地 MCP `--auth-*` profile 会在 callback 前持久化并复查授权并使用 SQLite authorization authority，显式 durable HTTP 与可信本地 MCP profile 则会选择完整 durable facade；opt-in durable retrieval-preparation bridge 会创建同一 scoped GateSession、保存并核验成对 evidence，再发布 `PREPARED`，durable Semantic Gate 组合则核验精确 evidence/attempt chain 并经 `AWAITING_DECISION` 发布 `DECIDED`，支持显式 failed-attempt retry 与 retained-success recovery；opt-in durable finalization 组合随后会复查同一 authorization event、active head 与 policy，保留精确 UsageDecision、injection 与 replay component，并通过 CAS 发布 `FINALIZED`；opt-in durable execution 组合会核验已保留 injection、要求当前 transition 授权、CAS 发布 `EXECUTING`、支持精确 resume/abandonment、认证 outcome evaluator，并组合原子 `COMPLETED` 与 completion-outbox emission；但带 peer authentication 的共享服务 transport、受保护内容加密、持久 transition-event linkage、shared-service worker、默认 adapter cutover 与 CLI durable 选择仍属于后续统一 schema 工作。既有 version-2 snapshot 中 verified 但未 review 的 case 必须补齐证据后才能加载。

以下 Phase 49–70 段落保留各功能落地时的版本与资源基线，属于历史兼容记录，其中出现的资源数量仍按各阶段当时的值解释，不随当前发布回写。当前发布契约以 PostgreSQL schema version 2 和 [`resources/manifest.json`](../resources/manifest.json) 的精确资源集合为准。历史上的第 21 项资源是面向既有 version-2 数据库、可重复运行且带版本门禁的 `schemas/postgres-v2-lock-order-hotfix.sql`；当前 `tbm.agent.v1` 资源包含十一份 JSON Schema、一份 OpenAPI、十一份 JSON 示例与一份可执行 quickstart；v3 资源包含 migration mapping/plan preflight、不可激活 bundle、隔离的 SQLite/PostgreSQL staging DDL、显式 PostgreSQL staging rollback、GateSession/内容寻址 injection/replay manifest/replay export 契约 Schema/示例、MemoryRevision approval/activation Schema/示例、隔离 SQLite MemoryRevision publication、GateSession/replay/audit/MemoryRevision/Artifact Authority/Semantic Gate/RunOutcome ledger DDL、隔离 PostgreSQL GateSession、MemoryRevision publication、replay、audit、authorization、Artifact Authority、Semantic Gate 与 RunOutcome ledger install/fail-closed rollback，以及授权 policy/decision Schema/示例。全新安装与当前 v1→v2 迁移已包含同一数据库修复。
