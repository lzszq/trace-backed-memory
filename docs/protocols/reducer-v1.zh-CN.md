...
依赖时才激活，Semantic provider effect 不支持 compensation。completion-provider integration、
自动 sweep/lease fencing、shared-
service worker 与 F2 其余 cutover gate 仍未关闭，因此这些 opt-in reducer 不会改变
`full_persistence=false`。
