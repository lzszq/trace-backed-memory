...
| `migration.snapshot-v3` | Migration | Snapshot v2/SQLite v1-to-v3 plan, bundle, apply, verify, and rollback | `opt-in` | Local SQLite migration is executable and restart-safe; it retains legacy records as unpublished/partial evidence. PostgreSQL cutover and ActivatedRevision publication remain outstanding. |
| `persistence.unified-sqlite-v3` | Persistence cutover | Unified SQLite v3 schema | `opt-in` | One generated bundle installs and fingerprints all 16 components: durable authority schemas plus the migration ledger. The compatibility schema remains SQLite 1. |
