...
| `protocol.provider-effect-ledger-v1` | Effect evidence | Content-addressed provider receipt and unknown-result recovery | `opt-in` | One strict provider-transition event, `effect-queue` reducer version 2, and an authenticated generic-ledger service retain attempts, provider request IDs, receipts, unknown results, reconciliation, explicit retry scheduling, and exact response-loss replay. Active semantic/completion callbacks and provider-specific reconciliation adapters do not yet select it. |
