...
| `protocol.event-type-registry-v1` | Full Persistence protocol | Sealed typed event registry, payload schemas, and upcasters | `contract-only` | Unknown types/versions remain preservable but cannot be silently consumed; the sealed default registry now contains 30 typed Gate, finalization, outcome, and effect events. Generic and domain reducers can bind it, while operator activation remains explicit and fail closed. |
| `protocol.provider-effect-ledger-v1` | Effect evidence | Content-addressed provider receipt and unknown-result recovery | `opt-in` | One strict provider-transition event, `effect-queue` reducer version 2, and an authenticated generic-ledger service retain attempts, provider request IDs, receipts, unknown results, reconciliation, explicit retry scheduling, and exact response-loss replay. Configured explicit durable runtimes select server-owned Semantic invocation plus a trusted reconciliation callback and bind the complete structured result digest. In-flight/submitted work remains recovery-required without mutation until durable owner-abandonment evidence exists. Completion-provider integration, request-only claims, active retry/dead-letter/compensation, concrete remote-provider adapters, PostgreSQL crash parity, and full transport parity remain open. |
