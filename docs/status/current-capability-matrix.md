...
| `protocol.trace-event-v1` | Trace evidence | Ordered, Artifact-linked engineering observations | `opt-in` | One typed observation family binds exact Trace/run sequence, adapter-supplied canonical times, Artifact-only content references, tool/permission/parent/subagent linkage, and a shared command digest for atomic batches of at most 100 events. The typed append helper verifies the command and ledger context before generic SQLite/PostgreSQL retention/replay. Codex Hook/App Server ingestion, Git observations, Trace reducers, and default cutover remain planned. |
| `protocol.event-type-registry-v1` | Full Persistence protocol | Sealed typed event registry, payload schemas, and upcasters | `contract-only` | Unknown types/versions remain preservable but cannot be silently consumed; the sealed default registry now contains 31 typed Gate, Trace, finalization, outcome, and effect events. Generic and domain reducers can bind it, while operator activation remains explicit and fail closed. |
