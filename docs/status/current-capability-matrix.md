...
| `protocol.event-type-registry-v1` | Full Persistence protocol | Sealed typed event registry, payload schemas, and upcasters | `contract-only` | Unknown types/versions remain preservable but cannot be silently consumed; the sealed default registry now contains 29 typed Gate, finalization, outcome, and effect events. Generic and domain reducers can bind it, while operator activation remains explicit and fail closed. |
