...
| `completion.outbox-v3` | 完成 | Outcome、outbox 与本地 effect authority/worker | `opt-in` | 显式 durable SQLite/PostgreSQL completion 会追加 `EffectRequested`；worker transition 会追加 canonical started/succeeded/failed/retry/dead-letter evidence，`effect-queue` 会重建精确 delivery history。`tbmd local` 运行有界 SQLite page。completion-provider receipt/compensation integration 与 shared-service dispatch 仍待完成；本地 `EffectSucceeded` 不是远端 receipt。 |
