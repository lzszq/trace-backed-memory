...
| `protocol.trace-event-v1` | Trace evidence | 有序、Artifact-linked 工程 observation | `opt-in` | 一个 typed observation family 会绑定精确 Trace/run sequence、adapter 提交的规范时间、仅 Artifact 的内容 reference、tool/permission/parent/subagent linkage，以及至多 100 条 event 原子批次共享的 command digest。typed append helper 会先核验 command 与 ledger context，再由 generic SQLite/PostgreSQL ledger 保留并重放。Codex Hook/App Server ingestion、Git observation、Trace reducer 与默认 cutover 仍待完成。 |
| `protocol.event-type-registry-v1` | Full Persistence 协议 | sealed typed event registry、payload schema 与 upcaster | `contract-only` | 未知 type/version 可以保留，但不能被静默消费；sealed 默认 registry 现包含 31 个 typed Gate、Trace、finalization、outcome 与 effect event。generic/domain reducer 均可绑定它，operator activation 保持显式并 fail closed。 |
