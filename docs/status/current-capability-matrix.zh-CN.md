...
| `completion.outbox-v3` | 完成 | Outcome、outbox 与本地 effect authority/worker | `opt-in` | 显式 durable SQLite/PostgreSQL completion 会追加 `EffectRequested`；worker transition 会追加 canonical started/succeeded/failed/retry/dead-letter evidence，`effect-queue` 会重建精确 delivery history。`tbmd local` 运行有界 SQLite page。operator 提供的 completion-provider consumer bridge 现已记录 lease/head-fenced provider receipt 与保守 unknown reconciliation；completion compensation、默认 bridge 构造与 shared-service dispatch 仍待完成。本地 `EffectSucceeded` 不是远端 receipt。 |
...
| `protocol.trace-event-v1` | Trace evidence | 有序、Artifact-linked 工程 observation | `opt-in` | 一个 typed observation family 会绑定精确 Trace/run sequence、adapter 提交的规范时间、仅 Artifact 的内容 reference、tool/permission/parent/subagent linkage，以及至多 100 条 event 原子批次共享的 command digest。typed append helper 会先核验 command 与 ledger context，再由 generic SQLite/PostgreSQL ledger 保留并重放。opt-in 固定 Codex App Server v2 adapter 现已把 Hook/diff/final-response notification 映射进该 family；Trace reducer 与默认 cutover 仍待完成。 |
| `protocol.git-observation-v1` | Git evidence | typed checkout/ref/commit/worktree/diff/ancestry/object/shallow observation | `opt-in` | opt-in recorder 会保持既有 `TraceMetadata` 与 `CommitAncestryEvidence` 返回契约不变，同时通过 generic SQLite/PostgreSQL ledger 追加八种 partition-scoped typed observation。payload 排除 raw path、remote URL、diff 与进程输出；精确 diff 字节只通过 Artifact 关联，缺失或不可核验的 ancestry 保持 unknown。adapter 必须跨有界批次串行化或预留 global position。自动 Git/diff capture、checkout authority、force-push reconciliation 与默认 cutover 仍待完成。 |
| `projection.git-graph-v1` | Git projection | conservative 可重建 commit/checkout/ref/relation/missing-object view | `opt-in` | 默认 reducer registry 可把 strict-scope Git observation 重建为 deterministic commit node、checkout/ref history、pairwise ancestry confidence/conflict、current missing-object state 与 last-observation provenance。矛盾保持 unknown；绝不推断 direct parent、force-push、source/fix/verification role 或 PR anchor。注册会轮换默认 reducer-registry digest，所以旧 default-registry checkpoint 必须使用新 rebuild generation。active retrieval/PR-risk consumer 尚未选择该 projection。 |
| `protocol.event-type-registry-v1` | Full Persistence 协议 | sealed typed event registry、payload schema 与 upcaster | `contract-only` | 未知 type/version 可以保留，但不能被静默消费；sealed 默认 registry 现包含 39 个 typed Gate、Trace、Git、finalization、outcome 与 effect event。generic/domain reducer 均可绑定它，operator activation 保持显式并 fail closed。 |
| `protocol.provider-effect-ledger-v1` | Effect evidence | 内容寻址 provider receipt 与 unknown-result recovery | `opt-in` | 严格 provider-transition event、`effect-queue` reducer version 3 与 authenticated generic-ledger service 会保留 provider-bound attempt/receipt、unknown result、trusted reconciliation、已保留 retry time、有界 dead-letter、每个 original effect 仅一条 compensation，以及 receipt-backed generic compensation。配置后的显式 durable runtime 还提供 provider/policy-bound Semantic request、稳定 provider 幂等键、request-only 原子 claim，以及 Python/HTTP/MCP/TypeScript invocation parity；operator 提供的 completion consumer 增加精确 worker-lease/stream-head fencing、fenced-owner unknown reconciliation 与 receipt replay。Semantic 与 completion provider effect 都不支持 compensation。具体 remote adapter、默认 completion-bridge 构造、自动 background sweep/lease fencing、shared-service worker 与其余 crash matrix 仍未完成。PostgreSQL provider crash probe 已加入但本机未运行；不声明 remote exactly-once。 |
...
| `integration.codex-hooks` | 工程集成 | Codex hooks/App Server adapter | `opt-in` | 可信 Codex CLI `0.146.0` App Server v2 recorder 会把固定 Hook lifecycle、turn diff 与 final response 映射为 exact-frame Artifact-linked Trace event，并执行严格拒绝与 pending-resume recovery。默认 Agent/MCP/HTTP profile 不会自动接线，direct Hook stdin 仍不支持。 |
