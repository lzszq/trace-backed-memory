...
whitespace-only content. A Store can therefore accept a record or snapshot that
later fails during `PostgresMemoryRepository.sync()`.

Phase 49 aligns the portable layers with constraints already present in the
database. A field covered by the phase must contain at least one non-whitespace
character. Python uses the project's existing `str.strip()` convention, JSON
Schema uses the existing `pattern: "\\S"` convention, and PostgreSQL keeps its
existing `btrim` or non-whitespace regular-expression checks.
...
does not change because fresh and existing schema-version-1 databases already
enforce the target contract. Six canonical/package Schema byte pairs change;
the packaged resource names and count remain 18.

## Compatibility

Previously accepted whitespace-only values in the listed Store, snapshot, or
context fields become invalid. They were never portable to the supported
PostgreSQL backend. Accepted strings are preserved byte-for-byte; the Store
does not trim them. Public signatures, dependencies, models, snapshot shape and
version 2, active-lessons YAML, PostgreSQL DDL, and PostgreSQL schema version 1
remain unchanged.
...
- PostgreSQL contract tests confirm the existing DDL checks remain byte-for-byte
  unchanged and reject representative whitespace values on a real cluster.
