...
[`resources/manifest.json`](../resources/manifest.json) 中的 entry 是唯一安装
资源 allowlist。每个安装副本必须与顶层编辑源字节一致；wheel 与 sdist 验证应在
缺失、额外或内容变化时失败。`PackagedResource` metadata 来自安装字节，包含
SHA-256 与大小。无路径 `load_failure_taxonomy()` 使用包内规范 taxonomy；显式
路径仍按调用方文档处理。白名单包含 fresh-install PostgreSQL schema 版本 2、独立
原子 `schemas/postgres-v1-to-v2.sql` migration、可重复执行的
`schemas/postgres-v2-lock-order-hotfix.sql` operator 脚本、`tbm.agent.v1`
请求/响应 Schema、OpenAPI、JSON 示例/quickstart，以及 v3 migration、GateSession、
replay、authorization、evidence、revision、retrieval 和隔离 authority 资源。
...
保持一致。契约变更不得静默拓宽 request model、HTTP route、MCP tool、body budget、
authentication boundary 或进程内 lifecycle 保证。
...
不得把合法 revision hash 当作 approval 或 activation。storage-neutral
approval/activation contract 要求独立 actor、精确 artifact/evidence replay、精确
`memory:review`/`memory:activate` authorization decision 与线性 immediate-
predecessor linkage。activation 必须重新验证完整 approval 输入；孤立的、内容合法的
approval event 仍不足以激活。publication authority 必须锁定并验证 durable current
head，因为调用方传入的 predecessor event 不能证明 currentness。禁止 global
MemoryRevision publication，也禁止在 revision chain 内 relocate target。修正必须
创建新 revision，不能修改既有 revision。
...
在 opt-in SQLite 或隔离 PostgreSQL v3 路径中，只能在执行产生显式
measurement 后构造 `GateCompletionRequest`，并针对与 executing GateSession
共用同一 connection 的 `SQLiteOutcomeV3Repository` 或
`PostgresOutcomeV3Repository` 调用
`GateSessionCompletionService.complete()`。caller 不得提供
session/Trace/run/usage linkage 或 measurement timestamp：authority 必须从
durable state 派生 linkage，并用 adapter 可信时间同时生成 outcome 与
...
使用 opt-in completion publication 时，应调用
...
可移植导出必须先授权 manifest 及其每个 artifact，再调用
`export_replay_bundle()`。传入非空 `allowed_classifications` frozenset，并通常设置
更低的调用方专用 `max_content_bytes`；两者都不会授予访问权限。导出的 JSON 应按敏感
evidence 处理并保留 `export_sha256`，import 或分析前使用
