...
classification metadata 本身不执行安全策略。opt-in 本地 Artifact Authority 会通过
调用方 provider 加密所有接受的 classification、授权每次读写，并执行读取时
retention/legal hold；它尚不执行物理清除、redaction 或密钥销毁。不得记录内容。
