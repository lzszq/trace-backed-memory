...
def test_repository_load_reproduces_derived_memory_run_remediations(
    postgres_cluster,
):
    psycopg = pytest.importorskip("psycopg")
    from trace_backed_memory.postgres import PostgresMemoryRepository
    ...
    store = _pending_memory_run_store()
    store.complete_trace("trace_atomic_run", eval_result="error")
    expected = store.memory_run_remediations()
    ...
    assert [item.action for item in expected] == [
        "investigate",
        "recover_with_attribution",
    ]
    assert restored.memory_run_remediations() == expected
...
def test_repository_load_reproduces_derived_memory_run_metrics(postgres_cluster):
    ...
    assert expected.auto_recoverable_count == 0
    assert expected.attribution_required_count == 1
