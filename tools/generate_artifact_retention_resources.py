...
TARGETS = {
    ...
    ROOT / "examples" / "artifact_retention_event_type_registry_v1.example.json": (
        lambda: json.dumps(
            build_artifact_retention_event_registry().catalog(),
            ensure_ascii=False,
            allow_nan=False,
            sort_keys=True,
            indent=2,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate deterministic artifact-retention schema/catalog resources."
    )
    parser.add_argument(
        "--write",
        action="store_true",
