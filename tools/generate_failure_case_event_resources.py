...
TARGETS = {
    ROOT / "schemas" / "failure_case_event_payload_registry_v1.schema.json": (
        dumps_failure_case_event_payload_dispatch_schema
    ),
    ROOT / "examples" / "failure_case_event_type_registry_v1.example.json": (
        lambda: json.dumps(
            build_failure_case_event_registry().catalog(),
            ensure_ascii=False,
            allow_nan=False,
            sort_keys=True,
            indent=2,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate deterministic FailureCase event resources."
    )
    parser.add_argument(
        "--write",
        action="store_true",
