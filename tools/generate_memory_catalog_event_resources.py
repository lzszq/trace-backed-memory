...
TARGETS = {
    ROOT / "schemas" / "memory_catalog_event_payload_registry_v1.schema.json": (
        dumps_memory_catalog_event_payload_dispatch_schema
    ),
    ROOT / "examples" / "memory_catalog_event_type_registry_v1.example.json": (
        lambda: json.dumps(
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate deterministic MemoryCatalog event resources."
    )
    parser.add_argument(
        "--write",
        action="store_true",
